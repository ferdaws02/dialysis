...
$RV=htmlspecialchars($_POST['RV']);
$DV=htmlspecialchars($_POST['DV']);
 $id_med=htmlspecialchars($_POST['id_med']);
 $id_p=htmlspecialchars($_POST['id_p']);
 
	 $sql=$bdd->prepare("INSERT INTO vaccin (Rappel_vaccin,D_Vacc,id_p,id_med)VALUES(?,?,?,?) "); 
    $sql->execute(array($RV,$DV,$id_p,$id_med));
...
$bdd=new PDO('mysql:host=dialyskfer.mysql.db;dbname=dialyskfer;charset=utf8', 'dialyskfer', 'Ons2017ie');
 $id_med=htmlspecialchars($_POST['id_med']);
 $id_p=htmlspecialchars($_POST['id_p']);
$sql=$bdd->prepare('SELECT * FROM vaccin ,patients WHERE vaccin.id_p=patients.id_p and vaccin.id_p=? and vaccin.id_med=? '); 

    $sql->execute(array($id_p,$id_med));
